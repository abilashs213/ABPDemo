...
docker build -f Dockerfile.local -t abp/demo-db-migrator:$version .
...
dotnet publish -c Release
docker build -f Dockerfile.local -t abp/demo-gateway-web:$version .

### PUBLICWEB-GATEWAY
Write-Host "*** BUILDING WEB-PUBLIC-GATEWAY ***" -ForegroundColor Green
Set-Location $webPublicGatewayFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t abp/demo-gateway-web-public:$version .
...
dotnet publish -c Release
docker build -f Dockerfile.local -t abp/demo-app-authserver:$version .

### PUBLIC-WEB
Write-Host "*** BUILDING WEB-PUBLIC ***" -ForegroundColor Green
Set-Location $publicWebFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t abp/demo-app-public-web:$version .
### Angular WEB App
if (Test-Path -Path $angularAppFolder) {
    ...
    # ng build --prod
    npm run build:prod
    docker build -f Dockerfile.local -t abp/demo-app-angular:$version .
}
